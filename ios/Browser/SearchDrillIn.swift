...
/// playable stream; `url` is a browsable container path. Abstracting these two
...
enum SearchDrillIn {
  ...
  ///
  /// If the first result is browsable-only (a `url` container with no `src` — a
  ...
  static func playable<Item: PlayableSearchItem>(
    ...
    let candidates: [Item]
    if let first = children.first, first.src == nil, let url = first.url {
      let drilled = try await resolveChildren(url).filter { $0.src != nil }
...
// The real Nitro `Track` already carries `src`/`url`; conform it for the app
