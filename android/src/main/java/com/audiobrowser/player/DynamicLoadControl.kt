 ...
 *
 * Unlike wrapping DefaultLoadControl, this implementation has its own mutable buffer
 * thresholds that can be changed at any time during playback. This allows adaptive
 * buffer management (via AutomaticBufferManager) to take effect immediately.
 *
 * Thread safety: LoadControl methods are called from ExoPlayer's internal playback thread,
 * while [updateBufferConfig] is typically called from the main thread. The @Volatile annotation
 * ensures visibility across threads.
 */
class DynamicLoadControl(
    initialConfig: BufferConfig = BufferConfig()
) : LoadControl {
    ...
    @Volatile
    private var lastRebufferLogTimeMs: Long = 0
    private val logIntervalMs = 1_000L // Log progress every 1 second

    // Track shouldStartPlayback call frequency
    @Volatile
    private var lastShouldStartCallMs: Long = 0
    @Volatile
    private var shouldStartCallCount: Int = 0
    @Volatile
    private var lastShouldStartLogTimeMs: Long = 0

    // Track startup timing
    @Volatile
    private var prepareStartTimeMs: Long = 0
    @Volatile
    private var playbackStarted: Boolean = false

    // Mutable buffer thresholds - can be updated at runtime
    @Volatile
    private var minBufferUs: Long = initialConfig.minBufferMs * 1000L

    @Volatile
    private var maxBufferUs: Long = initialConfig.maxBufferMs * 1000L

    @Volatile
    private var bufferForPlaybackUs: Long = initialConfig.bufferForPlaybackMs * 1000L

    @Volatile
    private var bufferForPlaybackAfterRebufferUs: Long = initialConfig.bufferForPlaybackAfterRebufferMs * 1000L

    @Volatile
    private var backBufferUs: Long = initialConfig.backBufferMs * 1000L

    @Volatile
    private var currentConfig: BufferConfig = initialConfig
    ...
    override fun shouldContinueLoading(parameters: LoadControl.Parameters): Boolean {
        ...
        val targetBufferUs = if (parameters.playbackSpeed > 1) {
    ...
    override fun shouldStartPlayback(parameters: LoadControl.Parameters): Boolean {
        ...
        if (!rebuffering && (shouldStartCallCount == 1 || now - lastShouldStartLogTimeMs >= logIntervalMs)) {
            lastShouldStartLogTimeMs = now
            Timber.d("shouldStartPlayback: buffer=%dms, call #%d",
                bufferedDurationMs, shouldStartCallCount)
        }
        ...
        if (rebuffering) {
            ...
            val adjustedTargetUs = if (parameters.playbackSpeed > 1) {
            ...
            if (now - lastRebufferLogTimeMs >= logIntervalMs) {
                ...
                Timber.d("Rebuffer progress: %dms / %dms (%.0f%%)",
                    bufferedDurationMs, targetMs, (bufferedDurationMs.toFloat() / targetMs) * 100)
            }
            return shouldStart
        }
    ...
    private fun logStartupTime(bufferedDurationMs: Long) {
        if (!playbackStarted && prepareStartTimeMs > 0) {
            ...
            Timber.d("🎵 PLAYBACK STARTED: %dms startup time, %dms buffer", startupTimeMs, bufferedDurationMs)
