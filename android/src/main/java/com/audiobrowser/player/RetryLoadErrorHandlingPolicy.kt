...
class RetryLoadErrorHandlingPolicy(
  ...
  companion object {
    ...
    private val RETRYABLE_HTTP_STATUS_CODES = setOf(
      408, // Request Timeout
  ...
  private fun calculateBackoffDelay(errorCount: Int): Long {
    ...
    val exponentialDelay = INITIAL_RETRY_DELAY_MS * BACKOFF_MULTIPLIER.pow((errorCount - 1).toDouble())
    return min(exponentialDelay.toLong(), MAX_RETRY_DELAY_MS)
  }

  /**
   * Checks if an HTTP error is worth retrying based on status code.
   * Only server errors (5xx) and specific client errors (408, 429) are retryable.
   */
  ...
  override fun getRetryDelayMsFor(loadErrorInfo: LoadErrorHandlingPolicy.LoadErrorInfo): Long {
    ...
    return if (isRecoverableError) {
      ...
      Timber.d("Retrying after IO error (attempt $errorCount, delay ${delay}ms): ${exception.message}")
      delay
    } else {
