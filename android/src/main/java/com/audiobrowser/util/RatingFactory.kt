...
object RatingFactory {
  fun media3ToBridge(
    rating: Rating
  ): Variant_HeartRating_ThumbsRating_StarRating_PercentageRating? {
    return when (rating) {
  ...
  fun bridgeToMedia3(
    rating: Variant_HeartRating_ThumbsRating_StarRating_PercentageRating
  ): Rating? {
    return when (rating) {
      is Variant_HeartRating_ThumbsRating_StarRating_PercentageRating.First ->
        Media3HeartRating(rating.value.hasHeart)

      is Variant_HeartRating_ThumbsRating_StarRating_PercentageRating.Second ->
        Media3ThumbRating(rating.value.isThumbsUp)

      is Variant_HeartRating_ThumbsRating_StarRating_PercentageRating.Third ->
        Media3StarRating(5, rating.value.stars.toFloat())

      is Variant_HeartRating_ThumbsRating_StarRating_PercentageRating.Fourth ->
        Media3PercentRating(rating.value.percentage.toFloat())
    }
  }
}
