...
class EqualizerManager(val audioSessionId: Int) {
  ...
  /**
   * Cleans preset name by removing null terminators and whitespace. Android's Equalizer API
   * sometimes returns preset names with trailing null characters.
   */
  ...
  fun getSettings(): EqualizerSettings? {
    ...
    return try {
      ...
      val bandLevels =
        (0 until bandCount)
          .map { band -> eq.getBandLevel(band.toShort()).toDouble() }
          .toDoubleArray()

      val centerFrequencies =
        (0 until bandCount)
          .map { band -> eq.getCenterFreq(band.toShort()).toDouble() }
          .toDoubleArray()

      val presets =
        (0 until eq.numberOfPresets)
          .map { preset -> cleanPresetName(eq.getPresetName(preset.toShort())) }
          .toTypedArray()
    ...
    try {
      ...
      if (presetIndex != null) {
        ...
        val availablePresets =
          (0 until eq.numberOfPresets).joinToString(", ") { index ->
  ...
  private fun notifySettingsChanged() {
    getSettings()?.let { settings -> onSettingsChanged?.invoke(settings) }
  }
