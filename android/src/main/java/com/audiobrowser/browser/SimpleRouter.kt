...
class SimpleRouter {
    ...
    fun <T> findBestMatch(path: String, routes: Map<String, T>): Pair<String, RouteMatch>? {
        return routes.keys
            .mapNotNull { pattern -> 
                matchPattern(pattern, path)?.let { match -> pattern to match }
            }
    ...
    private fun matchPattern(pattern: String, path: String): RouteMatch? {
        ...
        if (patternSegments.isNotEmpty() && patternSegments.last() == "**") {
            ...
            for (i in 0 until patternSegments.size - 1) {
                ...
                updateSegmentCounts(patternSegments[i], constantSegments, parameterSegments, wildcardSegments)
            }
            ...
            for (i in patternSegments.indices) {
                ...
                val (constCount, paramCount, wildcardCount) = updateSegmentCounts(
                    patternSegments[i], constantSegments, parameterSegments, wildcardSegments
                )
        ...
        val specificity = (constantSegments * 1000) + 
    ...
    private fun updateSegmentCounts(
        ...
        return when {
            ...
            patternSegment == "*" -> 
                Triple(constantSegments, parameterSegments, wildcardSegments + 1)
            else -> 
                Triple(constantSegments + 1, parameterSegments, wildcardSegments)
        }
    }
}