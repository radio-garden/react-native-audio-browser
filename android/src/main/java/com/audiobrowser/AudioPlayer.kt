...
@Keep
@DoNotStrip
class AudioPlayer : HybridAudioPlayerSpec(), ServiceConnection {
  ...
  init {
    ...
    launchInScope {
      try {
        Timber.d("Attempting to auto-bind to existing AudioBrowserService from AudioPlayerModule")
        val intent = Intent(context, Service::class.java)
        val bound = context.bindService(intent, this@AudioPlayer, Context.BIND_AUTO_CREATE)
  ...
  val callbacks =
    object : Callbacks {
      ...
      override fun handleRemotePlay(): Boolean {
        val handled =
          this@AudioPlayer.handleRemotePlay?.let {
            it.invoke()
            true
          } ?: false
      ...
      override fun handleRemotePause(): Boolean {
        val handled =
          this@AudioPlayer.handleRemotePause?.let {
            it.invoke()
            true
          } ?: false
      ...
      override fun handleRemoteStop(): Boolean {
        val handled =
          this@AudioPlayer.handleRemoteStop?.let {
            it.invoke()
            true
          } ?: false
      ...
      override fun handleRemoteNext(): Boolean {
        val handled =
          this@AudioPlayer.handleRemoteNext?.let {
            it.invoke()
            true
          } ?: false
      ...
      override fun handleRemotePrevious(): Boolean {
        val handled =
          this@AudioPlayer.handleRemotePrevious?.let {
            it.invoke()
            true
          } ?: false
      ...
      override fun handleRemoteJumpForward(event: RemoteJumpForwardEvent): Boolean {
        val handled =
          this@AudioPlayer.handleRemoteJumpForward?.let {
            it.invoke(event)
            true
          } ?: false
      ...
      override fun handleRemoteJumpBackward(event: RemoteJumpBackwardEvent): Boolean {
        val handled =
          this@AudioPlayer.handleRemoteJumpBackward?.let {
            it.invoke(event)
            true
          } ?: false
      ...
      override fun handleRemoteSeek(event: RemoteSeekEvent): Boolean {
        val handled =
          this@AudioPlayer.handleRemoteSeek?.let {
            it.invoke(event)
            true
          } ?: false
