...
- **Siri funnel (~1668):** the `params` assembly moved *above* the gate check; the old `guard browser.browseGate == nil` became `let outcome = await browser.gateDecision(for: NativeGateRequest(reason: .search, path: nil, search: params))` → on `gated`, fires `browser.onGate(GateEvent(reason: .search))` then `completion(false)`. **The resume branch is untouched and still returns before the gate check** — resume is hearing, never gated; ordering preserved.
